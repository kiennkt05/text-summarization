...
import argparse
from PTM_vera.model.model_loader import load_model_and_tokenizer
from PTM_vera.finetune.peft_utils import apply_vera, save_model_adapters
from PTM_vera.finetune.dataset import load_and_prepare_dataset
from PTM_vera.finetune.trainer import get_trainer
from PTM_vera.model import config

def main():
    parser = argparse.ArgumentParser(description="Fine-tune PTM using Unsloth with VeRA")
    ...
    parser.add_argument("--save_path", type=str, default=f"{config.OUTPUT_DIR}/vera_model", help="Path to save the VeRA adapters")
    ...
    print("Applying VeRA adapters...")
    model = apply_vera(model)

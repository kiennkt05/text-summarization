...
from PTM.model.config import config
...
def evaluate_dataset(model, tokenizer, test_path):
    ...
    for article in tqdm(df['article']):
        pred = generate_summary(model, tokenizer, article, stream=False)
...
if __name__ == "__main__":
    ...
    if args.test_path:
        evaluate_dataset(model, tokenizer, args.test_path)
    elif args.text:
        print("\n--- Summary ---")
